...
const estadoAcademicoLabels: Record<string, { label: string; variant: 'default' | 'secondary' | 'destructive' | 'outline' }> = {
...
const estadoEmpLabels: Record<string, { label: string; bg: string; text: string }> = {
  EMPLEADO: { label: 'Empleado',          bg: 'bg-green-100 dark:bg-green-900/30', text: 'text-green-800 dark:text-green-300' },
  BUSCANDO: { label: 'Buscando empleo',   bg: 'bg-amber-100 dark:bg-amber-900/30', text: 'text-amber-800 dark:text-amber-300' },
  SIN_INFO: { label: 'Sin información',   bg: 'bg-slate-100 dark:bg-slate-800',    text: 'text-slate-600 dark:text-slate-300' },
}
...
export default function EstudiantesPage() {
  ...
  // ── Cargar estudiantes ────────────────────────────────────────────────────
  const loadEstudiantes = useCallback(async (pgmId: string, pg: number) => {
    if (!pgmId) return
  ...
  useEffect(() => {
    if (selectedPgm) { setCurrentPage(0); loadEstudiantes(selectedPgm, 0) }
  }, [selectedPgm, loadEstudiantes])

  // ── Matches ───────────────────────────────────────────────────────────────
  const loadMatches = async (estudianteId: string) => {
    setLoadingMatches(true)
    try {
      const res = await matchesApi.listarPorEstudiante(estudianteId, 0, 50)
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        ...
        setTimeout(() => { setShowForm(false); loadEstudiantes(selectedPgm, currentPage) }, 800)
      } catch (err) {
        if (err instanceof ApiCallError) {
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      setDeleting(null); loadEstudiantes(selectedPgm, currentPage)
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <Button variant="outline" size="sm" onClick={() => loadEstudiantes(selectedPgm, currentPage)} className="shrink-0">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-border">
                    {filtered.map((est) => {
                      const ai = estadoAcademicoLabels[est.estadoAcademico] ?? { label: est.estadoAcademico, variant: 'secondary' as const }
                      const ei = estadoEmpLabels[est.estadoEmpleabilidad] ?? { label: est.estadoEmpleabilidad, bg: 'bg-slate-100', text: 'text-slate-600' }
                      return (
                        <tr key={est.id} onClick={() => openDetails(est)} className="hover:bg-secondary/30 transition-colors cursor-pointer">
                          ...
                          <td className="px-4 py-3"><Badge variant={ai.variant}>{ai.label}</Badge></td>
                          <td className="px-4 py-3">
                            <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${ei.bg} ${ei.text}`}>{ei.label}</span>
                          </td>
                ...
                <div className="flex items-center justify-between border-t border-border px-4 py-3">
                  ...
                  <div className="flex gap-1">
                    <button type="button" disabled={page.number === 0}
                      onClick={() => { const p = currentPage - 1; setCurrentPage(p); loadEstudiantes(selectedPgm, p) }}
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex gap-1.5 mt-2 flex-wrap">
                      <Badge variant={estadoAcademicoLabels[selected.estadoAcademico]?.variant ?? 'secondary'}>{estadoAcademicoLabels[selected.estadoAcademico]?.label ?? selected.estadoAcademico}</Badge>
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${estadoEmpLabels[selected.estadoEmpleabilidad]?.bg ?? ''} ${estadoEmpLabels[selected.estadoEmpleabilidad]?.text ?? ''}`}>
                        {estadoEmpLabels[selected.estadoEmpleabilidad]?.label ?? selected.estadoEmpleabilidad}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {detailTab === 'matches' && (
                  <div className="flex flex-col gap-4">
                    {loadingMatches ? (
                      ...
                      matches.map((m) => (
                        <div key={m.id} className="bg-card border border-border rounded-xl p-4 shadow-sm flex items-start gap-4 hover:shadow-md transition-shadow">
