...
export function Header({ onToggleSidebar, onOpenMobile }: HeaderProps) {
  ...
  // Las notificaciones siguen siendo mock hasta que se implemente
  // el módulo de notificaciones de estudiante.
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-navy-100 bg-card px-4 md:px-6">
      {/* Botón menú móvil */}
      <button
        type="button"
      ...
      <div className="relative ml-auto hidden max-w-md flex-1 sm:block">
        ...
        <Input
          type="search"
          placeholder="Buscar estudiantes, proyectos, documentos..."
          aria-label="Buscar"
      ...
      <button
        type="button"
        aria-label="Buscar"
        className="ml-auto flex size-9 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground sm:hidden"
      >
