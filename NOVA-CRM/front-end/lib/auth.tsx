...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(async (email: string, password: string) => {
    ...
    if (typeof window !== 'undefined') {
      // max-age alineado con la expiración del JWT del backend (15 min):
      // así el middleware redirige a /login cuando el token ya no sirve.
      document.cookie = `nova_token=${response.token}; path=/; max-age=900; SameSite=Lax`
    }
