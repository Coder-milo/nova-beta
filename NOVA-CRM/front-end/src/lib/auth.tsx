...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('nova:token-refreshed', handleTokenRefresh)
    return () => window.removeEventListener('nova:token-refreshed', handleTokenRefresh)
  }, [])
  ...
  const login = useCallback(async (email: string, password: string): Promise<AuthUser> => {
    ...
    if (typeof window !== 'undefined') {
      document.cookie = `nova_token=${response.token}; path=/; max-age=28800; SameSite=Lax`
    }
