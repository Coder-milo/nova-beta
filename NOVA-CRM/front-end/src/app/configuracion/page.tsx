...
export default function ConfiguracionPage() {
  ...
  const [instData, setInstData] = useState({
    nombreOficial: 'Academia CAC - Centro de Capacitación y Empleabilidad',
    nit: '901.452.839-4',
  ...
  const [integrationData, setIntegrationData] = useState({
    ...
    smtpPort: '587',
    smtpUser: 'apikey',
    smtpFromEmail: 'notificaciones@academia.edu.co',
  ...
  useEffect(() => {
    const saved = (localStorage.getItem('nova_theme') as 'light' | 'dark' | 'system') || 'light'
    setTheme(saved)
    applyTheme(saved)
  }, [])
            ...
            <CardHeader className="border-b border-border/50">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Gestión de cuentas con acceso al CRM de la Academia CAC.</CardDescription>
                </div>
                {!sinPermisoUsuarios && (
      ...
      {activeTab === 'mantenimiento' && (
        <div className="flex flex-col gap-6">
          ...
          <Card className="rounded-2xl shadow-sm">
            <CardHeader className="border-b border-border/50">
              <CardTitle className="flex items-center gap-2 text-base">
                <Palette className="size-5 text-primary" /> Apariencia Visual del CRM
              </CardTitle>
              <CardDescription>Selecciona la modalidad de visualización de la interfaz.</CardDescription>
            </CardHeader>
          ...
          {user?.roles?.includes('ADMIN') && (
            <Card className="rounded-2xl shadow-sm border-destructive/30">
              <CardHeader className="border-b border-border/50">
                ...
                <CardDescription>
                  Operaciones masivas de limpieza de datos en la base de datos de la Academia CAC.
                </CardDescription>
              </CardHeader>
              <CardContent className="flex flex-col gap-6 pt-6">
