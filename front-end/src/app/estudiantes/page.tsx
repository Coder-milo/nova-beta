          ...
          <CardContent>
            ...
            <div className="flex border-b border-border mb-5 -mx-6 px-6 gap-1">
              {([
                ...
                ['socio', 'Socioeconómico y Metas', CurrencyDollar],
            ...
            <form onSubmit={handleSave} className="flex flex-col gap-5">
              ...
              <div className="col-span-full flex justify-end gap-2 pt-2 border-t border-border">
                ...
                <Button type="submit" disabled={isPending}>
                  {isPending ? <><CircleNotch className="size-4 animate-spin" /> Guardando…</> : formMode === 'create' ? 'Registrar' : 'Actualizar'}
                </Button>
              </div>
            </form>
          </CardContent>
                          ...
                          <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                            <div className="inline-flex gap-1">
                              {verPapelera ? (
                                ...
                                <>
                                  <Link href={`/estudiantes/${est.id}`} onClick={(e) => e.stopPropagation()} title="Ver perfil completo" aria-label={`Ver perfil de ${est.nombre}`}
          ...
          <div className="bg-card rounded-xl border border-border shadow-lg p-6 max-w-sm w-full flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-150" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex justify-end gap-2 border-t border-border pt-3">
              ...
              <Button variant="destructive" onClick={executeDelete} disabled={deletingBusy}>
                {deletingBusy ? <><CircleNotch className="size-4 animate-spin" /> Eliminando…</> : 'Eliminar'}
              </Button>
            </div>
          </div>
        </div>
