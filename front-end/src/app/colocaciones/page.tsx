...
/**
 * El codigo viaja al backend; la etiqueta es interfaz. Estaban juntos en una
 * constante de modulo, y por eso los desplegables seguian en espanol con la
 * interfaz en ingles.
 */
function tiposDeVinculacion(T: ReturnType<typeof textos>, C: TextosAdmin) {
  return [['EMPLEADO', C.empleado], ['PRACTICANTE', T.practicante], ['APRENDIZ', T.aprendizSena],
          ['CONTRATISTA', T.prestacionDeServicios], ['FORMACION', T.vinculadoAFormacion]]
}

function canalesDeConsecucion(T: ReturnType<typeof textos>) {
  return [['OPEN_HOUSE', T.openHouse], ['VISITA_CAC', T.visitaCac], ['FERIA', T.feriaDeEmpleo],
          ['ALIADO', T.empresaAliada], ['PORTAL', T.portalVacanteDel], ['LINKEDIN', 'LinkedIn'],
          ['AUTOGESTIONADO', T.autogestionado], ['OTRO', T.otro]]
}

function dinero(valor: number | null, sinSalario: string) { return valor == null ? sinSalario : new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }).format(valor) }
...
export default function ColocacionesPage() {
  ...
  const guardar = async (event: React.SyntheticEvent) => {
    ...
    } catch (err) { setError(mensajeError(err, T.noSePudoX)) }
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm text-muted-foreground">{T.seguimientoDeEmpleabilidad}</p><p className="text-sm text-muted-foreground">{T.vinculacionesVerificadas}</p></div><Button onClick={abrirRegistro}><Plus /> {T.registrarColocacion}</Button></div>
    ...
    {cargando ? <PageSpinner label={T.cargandoColocaciones} /> : <>
      {resumen && <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><Kpi label={T.colocacionesVigentes} value={resumen.total} icon={Briefcase} /><Kpi label={T.sobreLaMeta} value={resumen.sobreMeta} icon={CurrencyDollar} /><Kpi label={T.gestionadasPorAcademy} value={resumen.gestionadasPorElPrograma} icon={Users} /><Kpi label={T.checklistCompleto} value={resumen.checklistCompletos} icon={CheckCircle} /><Kpi label={T.salarioPromedio} value={dinero(resumen.salarioPromedio, T.sinSalarioRegistrado)} icon={CurrencyDollar} /></div>}
      <div className="relative max-w-xl"><MagnifyingGlass className="absolute left-3 top-3 size-4 text-muted-foreground" /><Input className="pl-9" value={q} onChange={(event) => setQ(event.target.value)} placeholder={T.buscarParticipanteEmpresa} /></div>
      <Card className="shadow-none"><CardHeader className="pb-3"><CardTitle className="text-base">{T.vinculacionesActivas(filtrados.length)}</CardTitle><CardDescription>{T.esteModuloReemplaza}</CardDescription></CardHeader><CardContent>{filtrados.length === 0 ? <div className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">{T.noHayColocaciones}</div> : <div className="divide-y rounded-xl border">{filtrados.map((registro) => <div key={registro.id} className="flex flex-col gap-3 p-4 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Link href={`/estudiantes/${registro.estudianteId}`} className="font-semibold text-foreground hover:text-primary hover:underline">{registro.estudianteNombre}</Link><Badge variant="outline" className="text-[10px]">{registro.tipoVinculacionEtiqueta}</Badge>{registro.gestionadaPorElPrograma && <span className="text-[10px] font-medium text-emerald-700">{T.gestionadaPorEl}</span>}</div><p className="mt-1 text-sm text-foreground">{registro.empresaNombre}{registro.cargo ? ` · ${registro.cargo}` : ''}</p><p className="mt-1 text-xs text-muted-foreground">{T.inicioX(registro.fechaInicio || T.porConfirmar)} · {registro.canalConsecucionEtiqueta || T.canalSinRegistrar} · {registro.modalidad || T.modalidadSinRegistrar}</p><p className="mt-1 text-xs text-muted-foreground">{T.checklistX(registro.checklistVerificados, registro.checklistTotal, registro.checklistResumen)}</p></div><div className="flex items-center gap-3 lg:flex-col lg:items-end"><p className="text-sm font-semibold">{dinero(registro.salario, T.sinSalarioRegistrado)}</p><Button variant="outline" size="sm" onClick={() => void cerrar(registro)}><XCircle className="size-3.5" /> {C.cerrar}</Button></div></div>)}</div>}</CardContent></Card>
    </>}
    <Sheet open={abierto} onOpenChange={setAbierto}><SheetContent side="right" className="flex h-dvh w-full flex-col p-0 sm:max-w-2xl"><SheetHeader className="shrink-0 border-b border-border p-6"><SheetTitle>{T.registrarColocacion}</SheetTitle><SheetDescription>{T.guardaLaVinculacion}</SheetDescription></SheetHeader><form className="flex min-h-0 flex-1 flex-col" onSubmit={guardar}><div className="flex-1 overflow-y-auto p-6"><div className="space-y-6"><section className="space-y-3"><div><p className="text-sm font-semibold">{T.vinculacion}</p><p className="mt-1 text-xs text-muted-foreground">{T.identificaAlParticipante}</p></div><div className="grid gap-3 sm:grid-cols-2"><label className="sm:col-span-2"><span className="mb-1.5 block text-xs font-medium">{T.participante}</span><select required className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={form.estudianteId} onChange={(event) => setForm({ ...form, estudianteId: event.target.value })}><option value="">{T.seleccionaUnParticipante}</option>{estudiantes.map((estudiante) => <option key={estudiante.id} value={estudiante.id}>{estudiante.nombre} {estudiante.apellido} · {estudiante.nivelIngles || T.inglesSinRegistrar}</option>)}</select></label><label className="sm:col-span-2"><span className="mb-1.5 flex items-center gap-1.5 text-xs font-medium"><Buildings className="size-3.5 text-primary" />{T.empresa}</span><Input required list="empresas-colocacion" value={form.empresaNombre} onChange={(event) => setForm({ ...form, empresaNombre: event.target.value })} placeholder={T.seleccionaOEscribe} /><datalist id="empresas-colocacion">{empresas.map((empresa) => <option key={empresa.id} value={empresa.nombre}>{empresa.sector || T.sinSector}</option>)}</datalist><p className="mt-1 text-[11px] text-muted-foreground">{T.elListadoUsa}</p></label><label><span className="mb-1.5 block text-xs font-medium">{T.cargo}</span><Input value={form.cargo || ''} onChange={(event) => setForm({ ...form, cargo: event.target.value })} placeholder={T.cargoVinculado} /></label><label><span className="mb-1.5 block text-xs font-medium">{T.tipoDeVinculacion}</span><select className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={form.tipoVinculacion || ''} onChange={(event) => setForm({ ...form, tipoVinculacion: event.target.value })}>{tiposDeVinculacion(T, C).map(([valor, etiqueta]) => <option key={valor} value={valor}>{etiqueta}</option>)}</select></label></div></section><section className="space-y-3 border-t border-border pt-5"><div><p className="text-sm font-semibold">{T.condicionesLaborales}</p><p className="mt-1 text-xs text-muted-foreground">{T.estosDatosAlimentan}</p></div><div className="grid gap-3 sm:grid-cols-2"><label><span className="mb-1.5 block text-xs font-medium">{T.fechaDeInicio}</span><Input type="date" value={form.fechaInicio || ''} onChange={(event) => setForm({ ...form, fechaInicio: event.target.value })} /></label><label><span className="mb-1.5 block text-xs font-medium">{T.canalDeConsecucion}</span><select className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={form.canalConsecucion || ''} onChange={(event) => setForm({ ...form, canalConsecucion: event.target.value })}><option value="">{C.sinRegistrar}</option>{canalesDeConsecucion(T).map(([valor, etiqueta]) => <option key={valor} value={valor}>{etiqueta}</option>)}</select></label><label><span className="mb-1.5 block text-xs font-medium">{T.salarioMensualCop}</span><Input type="number" min="0" value={form.salario ?? ''} onChange={(event) => setForm({ ...form, salario: event.target.value ? Number(event.target.value) : null })} placeholder={T.ej2500000} /></label><label><span className="mb-1.5 block text-xs font-medium">{T.modalidad}</span><Input value={form.modalidad || ''} onChange={(event) => setForm({ ...form, modalidad: event.target.value })} placeholder={T.presencialRemotoO} /></label><label className="sm:col-span-2"><span className="mb-1.5 block text-xs font-medium">{T.tipoDeContrato}</span><Input value={form.tipoContrato || ''} onChange={(event) => setForm({ ...form, tipoContrato: event.target.value })} placeholder={T.indefinidoFijoPrestacion} /></label></div></section><section className="space-y-1.5 border-t border-border pt-5"><label><span className="block text-sm font-semibold">{T.observaciones}</span><span className="mt-1 block text-xs text-muted-foreground">{T.anadeCondicionesPor}</span><Textarea className="mt-3 w-full rounded-md border border-input bg-background p-3 text-sm" value={form.observaciones || ''} onChange={(event) => setForm({ ...form, observaciones: event.target.value })} placeholder={T.ejPendienteConfirmar} /></label></section></div></div><div className="flex shrink-0 justify-end gap-2 border-t border-border bg-background p-4"><Button type="button" variant="outline" onClick={() => setAbierto(false)}>{C.cancelar}</Button><Button type="submit" disabled={guardando}>{guardando && <CircleNotch className="size-4 animate-spin" />}{guardando ? C.guardando : T.registrarColocacion}</Button></div></form></SheetContent></Sheet>
    {dialogo}
  </div>
}
