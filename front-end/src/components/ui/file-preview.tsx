...
export function FilePreview({ archivo, nombre, contentType, className = '' }: {
  ...
  if (tipo === 'imagen') return <div className={`overflow-hidden rounded-xl border border-border bg-muted/30 ${className}`}><img src={url} alt={`Vista previa de ${nombre}`} className="max-h-72 w-full object-contain" /></div>
  if (tipo === 'pdf') return <iframe src={url} title={`Vista previa de ${nombre}`} className={`h-[28rem] w-full rounded-xl border border-border bg-white ${className}`} />
  ...
  return <div className={`flex items-center gap-3 rounded-xl border border-dashed border-border bg-muted/20 p-4 ${className}`}><span className="flex size-10 items-center justify-center rounded-lg bg-background"><FileText className="size-5 text-muted-foreground" /></span><div><p className="text-sm font-medium">Archivo listo para subir</p><p className="text-xs text-muted-foreground">Este formato se conserva intacto y se puede descargar después de subirlo.</p></div></div>
}
    ...
    apiBlob(endpoint)
      ...
      .catch((err) => {
        if (!cancelado) setError(err instanceof ApiCallError
          ? (err.body.message ?? `No se pudo previsualizar el archivo (HTTP ${err.status}).`)
