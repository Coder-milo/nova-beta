...
function textos(english: boolean) {
  return english
    ? {
        titulo: 'Possible duplicate records',
        desc: 'Companies whose name looks like another one already on file.',
        vacio: 'No duplicate-looking records.',
        registros: (n: number) => `${n} linked records`,
        seQueda: 'Keep this one',
        fusionar: 'Merge',
        fusionando: 'Merging…',
        confirmarTitulo: 'Merge these records?',
        confirmarTexto: (o: string, d: string, n: number) =>
          `«${o}» will be absorbed into «${d}»: ${n} records —vacancies, approaches, applications, placements and portal accounts— move over. The absorbed record is deactivated, not deleted. This cannot be undone.`,
        confirmarBoton: 'Merge',
        hecho: (n: number) => `Merged. ${n} records moved.`,
        fallo: 'They could not be merged.',
        aviso: 'Two names that look alike can still be two different companies of the same group. Check before merging.',
        cargando: 'Looking for duplicates…',
      }
    : {
        titulo: 'Posibles fichas duplicadas',
        desc: 'Empresas cuyo nombre se parece al de otra ya registrada.',
        vacio: 'No hay fichas que parezcan repetidas.',
        registros: (n: number) => `${n} registros asociados`,
        seQueda: 'Que se quede esta',
        fusionar: 'Fusionar',
        fusionando: 'Fusionando…',
        confirmarTitulo: '¿Unir estas fichas?',
        confirmarTexto: (o: string, d: string, n: number) =>
          `«${o}» se absorbe dentro de «${d}»: se mueven ${n} registros —vacantes, acercamientos, postulaciones, colocaciones y cuentas del portal—. La ficha absorbida se desactiva, no se borra. Esto no se puede deshacer.`,
        confirmarBoton: 'Fusionar',
        hecho: (n: number) => `Fusionadas. Se movieron ${n} registros.`,
        fallo: 'No se pudieron fusionar.',
        aviso: 'Dos nombres parecidos pueden ser dos empresas distintas del mismo grupo. Revísalo antes de unirlas.',
        cargando: 'Buscando duplicados…',
      }
}
