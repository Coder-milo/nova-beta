...
export function StudentHelpChat() {
  ...
  return <div className="fixed bottom-5 right-5 z-50 sm:bottom-7 sm:right-7">
    ...
    <button ref={triggerRef} type="button" onClick={() => setOpen((value) => !value)} aria-label={open ? 'Close Alex' : labels.title} title={open ? 'Close Alex' : labels.title} className="group relative flex size-14 items-center justify-center overflow-hidden rounded-[1.15rem] bg-primary/10 shadow-[0_16px_34px_-12px_color-mix(in_srgb,var(--primary)_80%,transparent)] ring-1 ring-primary/25 transition hover:scale-105 hover:ring-primary/45 active:scale-95"><span className="absolute inset-0 rounded-[1.15rem] bg-primary/25 blur-lg transition group-hover:blur-xl" /><img src="/brand/alex-fox.png" alt="" className="relative size-full object-contain" /></button>
  </div>
}
