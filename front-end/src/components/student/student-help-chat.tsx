  ...
  return (
    <>
      ...
      >
        ...
        >
          <header className="relative overflow-hidden border-b border-primary/15 bg-[linear-gradient(125deg,color-mix(in_srgb,var(--primary)_23%,transparent),transparent_62%)] px-4 py-3.5 dark:bg-[#13221d]">
            <div className="absolute -right-5 -top-8 size-28 rounded-full bg-primary/10 blur-2xl dark:hidden" />
            <div className="relative flex items-center gap-3">
              <span aria-hidden="true" className="flex size-10 items-center justify-center overflow-hidden rounded-2xl bg-primary/10 shadow-lg">
              ...
              <button type="button" onClick={limpiar} title={labels.clear} aria-label={labels.clear} className="rounded-xl p-2 text-muted-foreground transition hover:bg-background/70 hover:text-foreground dark:hover:bg-[#0c1714]">
                <Trash className="size-4" />
              </button>
              <button type="button" onClick={() => setOpen(false)} aria-label="Close" className="rounded-xl p-2 text-muted-foreground transition hover:bg-background/70 hover:text-foreground dark:hover:bg-[#0c1714]">
                <X className="size-4" />
              </button>
            </div>
          </header>
          <div className="flex-1 space-y-3 overflow-y-auto bg-[linear-gradient(180deg,color-mix(in_srgb,var(--primary)_3%,transparent),transparent_34%)] p-4 dark:bg-[#0c1714]">
            {messages.map((message, indice) => (
              <div key={message.id} className={cn('flex flex-col', message.author === 'student' ? 'items-end' : 'items-start')}>
                ...
                {message.author === 'bot' && indice === messages.length - 1 && !!message.sugerencias?.length && (
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {message.sugerencias.map((sugerencia) => (
                      <button key={sugerencia} type="button" onClick={() => enviar(sugerencia)} className="rounded-full border border-primary/20 px-2.5 py-1 text-[11px] font-medium text-primary transition hover:bg-primary/10">
                        {sugerencia}
                      </button>
                    ))}
                  </div>
            ...
            {typing && (
              <div className="flex">
                <div className="flex items-center gap-2 rounded-2xl rounded-tl-md border border-border bg-card px-3.5 py-2.5 text-xs text-muted-foreground dark:bg-[#13221d]">
                  <CircleNotch className="size-3.5 animate-spin text-primary" />
                  {english ? 'Preparing an answer...' : 'Preparando una respuesta...'}
                </div>
              </div>
            )}
            {messages.length <= 2 && (
              <div className="space-y-2 pt-1">
                <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">{english ? 'Quick actions' : 'Acciones rápidas'}</p>
                <div className="grid gap-2 sm:grid-cols-3">
                  {topics.map((topic) => {
                    const Icon = topic.icon
                    return (
                      <button key={topic.id} type="button" onClick={() => { enviar(topic.question); if (topic.route) router.push(topic.route) }} className="rounded-xl border border-border bg-card p-2.5 text-left text-[11px] font-medium text-foreground transition hover:border-primary/30 hover:bg-primary/[0.04]">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {labels.suggestions.map((suggestion) => (
                    <button key={suggestion} type="button" onClick={() => enviar(suggestion)} className="rounded-full border border-primary/20 px-2.5 py-1 text-[11px] font-medium text-primary transition hover:bg-primary/10">
                      {suggestion}
                    </button>
                  ))}
                </div>
          ...
          <div className="border-t border-border bg-card p-3 dark:bg-[#13221d]">
            <button type="button" onClick={contactar} className="mb-2 flex items-center gap-1.5 text-xs font-medium text-primary hover:underline">
