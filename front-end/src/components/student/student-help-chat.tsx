...
type AssistantResponse = { respuesta: string; accionNavegacion?: NavigationAction | null }
type Message = { id: string; author: Author; text: string; createdAt: number; accionNavegacion?: NavigationAction | null }
  ...
  return <div className="fixed bottom-5 right-5 z-50 sm:bottom-7 sm:right-7">
    {open && <section ref={chatRef} aria-label={labels.title} className="mb-3 flex h-[min(72dvh,620px)] w-[calc(100vw-2rem)] max-w-md flex-col overflow-hidden rounded-3xl border border-border bg-popover shadow-[0_28px_70px_rgba(0,0,0,0.32)] dark:bg-[#0c1714]">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto bg-[linear-gradient(180deg,color-mix(in_srgb,var(--primary)_3%,transparent),transparent_34%)] p-4 dark:bg-[#0c1714]">
        {messages.map((message) => <div key={message.id} className={cn('flex', message.author === 'student' && 'justify-end')}><div className={cn('max-w-[86%] rounded-2xl px-3.5 py-2.5 text-sm leading-5 shadow-sm', message.author === 'bot' ? 'rounded-tl-md border border-border bg-card text-foreground dark:bg-[#13221d]' : 'rounded-tr-md bg-primary text-primary-foreground')}><p>{message.text}</p>{message.author === 'bot' && message.accionNavegacion && <button type="button" onClick={() => { setOpen(false); router.push(message.accionNavegacion!.url) }} className="mt-2 rounded-lg border border-primary/25 bg-primary/10 px-2.5 py-1.5 text-xs font-semibold text-primary transition hover:bg-primary/15">{message.accionNavegacion.etiqueta}</button>}</div></div>)}
