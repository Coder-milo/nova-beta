...
public class DocumentoService {
    ...
    public Page<DocumentoResponse> buscar(UUID estudianteId, UUID programaId, boolean soloAdministrativos,
                                           String tipo, String q, Pageable pageable) {
    ...
    /** Documentos del estudiante autenticado; nunca acepta un id desde el cliente. */
    public Page<DocumentoResponse> buscarMios(String tipo, String q, Pageable pageable, Authentication auth) {
        var estudiante = ownershipService.obtenerEstudianteAutenticado(auth);
        return buscar(estudiante.getId(), null, false, tipo, q, pageable);
    }
    ...
    /**
     * Adjunta un documento a la propia ficha del estudiante. El programa y el
     * estudiante se resuelven desde la sesión para impedir asociaciones ajenas.
     */
    @Transactional
    public DocumentoResponse subirMio(MultipartFile archivo, String tipo, Authentication auth) {
        validarArchivo(archivo);
        var estudiante = ownershipService.obtenerEstudianteAutenticado(auth);
        var doc = new Documento();
        doc.setGrupoId(UUID.randomUUID());
        ...
        doc.setPrograma(estudiante.getPrograma());
        doc.setTipo(normalizarTipo(tipo));
        doc.setNombre(archivo.getOriginalFilename() != null ? archivo.getOriginalFilename() : "archivo");
        doc.setContentType(archivo.getContentType());
        doc.setTamano(archivo.getSize());
        doc.setSubidoPor(usuarioActual());
    ...
    public byte[] contenidoMio(UUID id, Authentication auth) {
        verificarEsPropio(id, auth);
        return contenido(id);
    }

    /** El estudiante puede borrar únicamente archivos que él mismo subió. */
    @Transactional
    public void eliminarMio(UUID id, Authentication auth) {
        var doc = verificarEsPropio(id, auth);
        if (doc.getSubidoPor() == null || !doc.getSubidoPor().equalsIgnoreCase(auth.getName())) {
            throw new AccessDeniedException("Solo puedes eliminar documentos que hayas subido tú");
        }
        eliminar(id);
    }
    ...
    private Documento verificarEsPropio(UUID id, Authentication auth) {
        var estudiante = ownershipService.obtenerEstudianteAutenticado(auth);
        var doc = obtenerEntidad(id);
        if (doc.getEstudiante() == null || !estudiante.getId().equals(doc.getEstudiante().getId())) {
            throw new AccessDeniedException("No puedes acceder a documentos de otro estudiante");
        }
        return doc;
    }
